...
def plot_propagation(date_sel,composite=True):
    ...
    Parameters
    ----------
    st : np.array with the positions of events from 1Jan and multiple of 365. Can use ssw365 directly.
    ...
    '''
    root = Dataset('C:/Users/Public/ERA5_data/daily/NAM_ERA5_std_new.nc')
    year0=1950
    iyear = year0-1959
    if iyear < 0:
        iyear = 0
    dat     = np.squeeze(root.variables['nam'][iyear*365::])*(1)
    lev     = root.variables['lev'][:]
    nz = len(lev)
    ## NOW let's do a composite
    mask = np.array([f.year > 1960 for f in date_sel])
    date_sel = date_sel[mask]
    date_sel = date_sel[:-1]
...
def plot_impacts(st1,date_sel,var,composite=True):
    ...
    '''    
    root    = Dataset('C:/Users/Public/ERA5_data/geopot/day/lr_geopot1000_day_ERA5_1950-2021.nc')
